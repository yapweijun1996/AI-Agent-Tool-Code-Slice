...
> Status: **V0.1 core Verified** (functional correctness and cross-platform package/install CI). The published package is `agent-code-slice@0.2.0`; hardening commit `1712f17` adds parser resource cleanup, bounded requests/results, and strict CLI validation. That commit passed cross-platform CI run [33972164494](https://github.com/yapweijun1996/AI-Agent-Tool-Code-Slice/actions/runs/33972164494), including all nine Windows/macOS/Ubuntu × Node 18.18.0/20/22 test jobs and three Node 20 benchmark jobs.
> The current Unreleased working-tree changes additionally fix BOM-safe byte ranges, whitespace-aware line/range container selection, and bounded error envelopes. Local verification is passing; cross-platform CI evidence for these changes is still pending.
> The published Core API, CLI, JS API, and JavaScript/TypeScript/TSX/Python/CFML adapters have a 47-test regression suite passing on Windows Server 2025, macOS 26, and Ubuntu 24.04 (Node 18.18.0, 20, and 22 each) — see the [CI workflow](.github/workflows/ci.yml) and `CHANGELOG.md` for the exact run. The hardening commit adds 55 unit tests, 16 Golden cases, and 3 deterministic agent-facing E2E cases. Release CI run [33937790994](https://github.com/yapweijun1996/AI-Agent-Tool-Code-Slice/actions/runs/33937790994) passed the published `0.2.0` package on the Windows/macOS/Ubuntu × Node 18.18.0/20/22 matrix and installed it from the public registry on all three platforms with Node 20. The benchmark (`docs/PERFORMANCE_BENCHMARK_RESULTS.md`) covers all current adapters and four sizes; the latest hardening run passed the Ubuntu/Windows/macOS × Node 20 benchmark jobs and uploaded one non-expired report artifact per job. The registry-install job was skipped for the unpublished hardening commit, so registry evidence remains tied to the published release. No MCP server is planned; the future serverless adapter and agent-specific integration packs remain **Planned** (V0.2).
